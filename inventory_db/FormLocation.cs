...
namespace inventory_db
{
    public partial class FormLocation : Form
    {
        ...
        private void buttonAddNewLocation_Click(object sender, EventArgs e)
        {
            ...
            command.Parameters.Add("@location_name", MySqlDbType.VarChar).Value = textBoxAddNewLocation.Text;
            ...
            commandDatabase.Parameters.Add("@location_name", MySqlDbType.VarChar).Value = locationName;
        ...
        private void RefreshlistViewLocation()
        {
            ...
            {
                sqlConnection.Open();
                MySqlCommand sqlCommand = new MySqlCommand("SELECT location_name " +
                                                           "FROM location ", sqlConnection);
                dataReader = sqlCommand.ExecuteReader();
                while (dataReader.Read())
                {
                    row = new string[]
                    {
                        Convert.ToString(dataReader["location_name"]),
                    };
        ...
        private void buttonDeleteUser_Click(object sender, EventArgs e)
        {
            if (this.listViewLocation.SelectedItems.Count != 0)
            {
                ...
                {
                    string query = "DELETE FROM `location` WHERE `location_name` = @location_name";
                    MySqlCommand commandDatabase = new MySqlCommand(query, sqlConnection);
                    commandDatabase.Parameters.Add("@location_name", MySqlDbType.VarChar).Value = locationMameMouse;
        ...
        private void methodChangeLocation()
        {
            ...
            else
            {
                ...
                string query = "UPDATE `location` " +
                    ...
                    "WHERE location_name = @location_old_name";

                MySqlCommand commandDatabase = new MySqlCommand(query, sqlConnection);
                commandDatabase.Parameters.Add("@location_name", MySqlDbType.VarChar).Value = location;
